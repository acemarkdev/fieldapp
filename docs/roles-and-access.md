...
- [ ] **E. Server endpoint checks** in the office API for defence in depth (the office server uses the service-role key, so this is the remaining gap for office-side writes).
